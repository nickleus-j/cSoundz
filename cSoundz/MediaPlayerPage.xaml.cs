...
namespace cSoundz
{
    ...
    public sealed partial class MediaPlayerPage : Page, IBarElementFactory
    {
        ...
        public MediaPlayerPage()
        {
            ...
            barspectrum.ElementShadowColor = Colors.SeaGreen;
            barColor = GetSolidColorBrush().Color;
        }
        private SolidColorBrush GetSolidColorBrush()
        {
            return new SolidColorBrush(Windows.UI.Color.FromArgb(Convert.ToByte(255), Convert.ToByte(rnd.Next(256)), Convert.ToByte(rnd.Next(256)), Convert.ToByte(rnd.Next(256))));
        }
        ...
        private void DrawSpectrum(object sender, AudioVisualizer.VisualizerDrawEventArgs args)
        {
            ...
            Rect boundingRectTop = new Rect(margin.Left, margin.Top, args.ViewExtent.Width - margin.Right - margin.Left, Math.Abs(barSize.Y));
            Rect boundingRectBottom = new Rect(margin.Left, margin.Top * 2 + margin.Bottom +Math.Abs(barSize.Y), args.ViewExtent.Width - margin.Right - margin.Left, Math.Abs(barSize.Y));
            ...
            for (int index = 0; index < spectrumBarCount; index++)
            {
                ...
                drawingSession.FillRectangle(barX, (float)boundingRectTop.Bottom - spectrumBarHeight1, barSize.X, spectrumBarHeight1, barColor);
                drawingSession.FillRectangle(barX, (float)boundingRectBottom.Bottom - spectrumBarHeight2, barSize.X, spectrumBarHeight2, barColor);
            }
            ...
            for (int index = 0; index < spectrumBarCount; index++)
            {
                ...
                if (index != 0)
                {
                    drawingSession.DrawLine(prevPointLeft, leftPoint, Colors.Purple, 3);
                    drawingSession.DrawLine(prevPointRight, rightPoint, Colors.DarkViolet, 3);
                }
